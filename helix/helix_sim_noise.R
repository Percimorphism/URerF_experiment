...
helix_noise_data=cbind(helix_data, noise)

#non_normalize vs normalize (without noise)
#first we do non_normalized
#URerF
g_noise=Urerf(helix_noise_data, trees = 300, Progress = TRUE, splitCrit = "bicfast", normalizeData = FALSE)
W_noise=g_noise$similarityMatrix
D_rf_noise=1-W_noise
# isomap
D_eucd_noise = as.matrix(dist(helix_noise_data))
iso_dist_noise = as.matrix(isomapdist(D_eucd_noise, k=9))
#UMAP
custom.settings = umap.defaults
custom.settings$n_neighbors=num_of_points
a_noise = umap(helix_noise_data, config = custom.settings)
D_umap_noise=as.matrix(dist(a_noise$layout))

############################################################################
#now we do normalized
norm_helix_noise_data=normalizeData(helix_noise_data)
#URerF norm
g_noise_norm=Urerf(norm_helix_noise_data, trees = 300, Progress = TRUE, splitCrit = "bicfast", normalizeData = FALSE)
W_noise_norm=g_noise_norm$similarityMatrix
D_rf_noise_norm=1-W_noise_norm
# isomap norm
D_eucd_noise_norm = as.matrix(dist(norm_helix_noise_data))
iso_dist_noise_norm = as.matrix(isomapdist(D_eucd_noise_norm, k=9))
#UMAP norm
a_noise_norm = umap(norm_helix_noise_data, config = custom.settings)
...
save(D_rf_noise_precision_list, D_rf_noise_norm_precision_list, 
     ...
     at_K, file="helix_noise_precision_list.Rdata")
